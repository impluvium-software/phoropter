...
`tests/fixtures/parity/*.json` are generated by the reference implementation's
`ParityFixtureEmitterTest` (in the `slicing-proof` repository):
...
(`generated_from.git_sha`); the parity gate asserts it is present. Regenerate fixtures
only when the reference slicer intentionally changes, and note the new SHA in the PR.
