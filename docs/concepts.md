...
This document is the normative reference for Phorapter's core mechanics: the
...
Phorapter indexes every document at **multiple slice sizes** simultaneously — the
...
(`PHORAPTER_NAMESPACE`, itself derived as
`uuid5(NAMESPACE_DNS, "phorapter.impluvium.software")` and pinned by test). The
