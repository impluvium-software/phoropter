...
prefixed ``PHORAPTER_`` — nested sections use a double underscore, e.g.
``PHORAPTER_STORE__URL`` — then from an optional ``phorapter.toml`` in the
...
from phorapter.tokens import DEFAULT_COUNTER_ID
...
class StoreSettings(BaseModel):
    """Vector store settings. ``kind`` is a name registered under ``phorapter.stores``."""
...
class EmbedderSettings(BaseModel):
    """Embedder settings. ``provider`` is a name registered under ``phorapter.embedders``."""
...
class Settings(BaseSettings):
    ...
    thread it through :class:`~phorapter.service.core.ServiceCore`.
    """

    model_config = SettingsConfigDict(
        env_prefix="PHORAPTER_",
        env_nested_delimiter="__",
        toml_file="phorapter.toml",
        extra="ignore",
    )
    ...
    def settings_customise_sources(
        ...
        # an optional phorapter.toml. The environment always beats the file.
        return (
            init_settings,
            env_settings,
...
def build_store(settings: Settings) -> VectorStoreAdapter:
    ...
    ``phorapter.stores`` entry-point group and constructed with no arguments.
    """
    from phorapter.stores import load_store_class
...
def build_embedder(settings: Settings) -> Embedder:
    """Construct the configured embedder from the ``phorapter.embedders`` registry.
    ...
    Unknown providers raise :class:`~phorapter.errors.EmbedderError`.
    """
    from phorapter.embed import create_embedder
