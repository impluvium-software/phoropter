...
:mod:`phorapter.server.mappers`. Every :class:`~phorapter.errors.PhorapterError`
...
from phorapter.config import build_embedder, build_store
...
from phorapter.server.middleware import (
...
def _register_handlers(app: FastAPI) -> None:
    @app.exception_handler(PhorapterError)
    async def _phorapter_error(request: Request, exc: PhorapterError) -> JSONResponse:
...
def _register_routes(app: FastAPI, service: ServiceCore) -> None:
    ...
    async def info() -> sc.Info:
        from phorapter.embed import default_registry
        from phorapter.stores import available_store_names
    ...
    async def batch_documents(
        ...
        if len(body.documents) > limit:
            from phorapter.service.documents import DocumentTooLargeError
