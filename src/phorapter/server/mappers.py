...
The DTOs in :mod:`phorapter.server.schemas` are generated from
...
:class:`~phorapter.selection.Selection` into the response verbatim, never
...
from phorapter.model import HitProvenance
from phorapter.server import schemas as sc

if TYPE_CHECKING:
    from phorapter.selection import EvidenceItem, SelectedSlice, Selection
    from phorapter.service.corpora import CorpusInfo
    from phorapter.service.query import QueryOutcome
    from phorapter.stores import DocumentPage, DocumentRecord
    from phorapter.trace import SubstitutionTrace
...
def corpus_info_to_dto(info: CorpusInfo) -> sc.Corpus:
    """Map a :class:`~phorapter.service.corpora.CorpusInfo` to the ``Corpus`` DTO."""
    cfg = info.config
    return sc.Corpus(
        name=cfg.name,
...
def document_record_to_dto(record: DocumentRecord) -> sc.DocumentRecord:
    """Map a :class:`~phorapter.stores.DocumentRecord` to its DTO."""
...
def document_page_to_dto(page: DocumentPage) -> sc.DocumentPage:
    """Map a :class:`~phorapter.stores.DocumentPage` to its DTO."""
...
def query_outcome_to_dto(outcome: QueryOutcome) -> sc.QueryResponse:
    """Map a :class:`~phorapter.service.query.QueryOutcome` to the ``QueryResponse`` DTO.
