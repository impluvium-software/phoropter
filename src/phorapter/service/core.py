...
exposes the three domain services (:class:`~phorapter.service.corpora.CorpusService`,
:class:`~phorapter.service.documents.DocumentService`,
:class:`~phorapter.service.query.QueryService`). Every I/O the surfaces need
...
(one :meth:`~phorapter.stores.VectorStoreAdapter.fetch_by_ids` per size) and
hands the engine a fixed, in-memory :class:`~phorapter.selection.SliceSource`.
"""
...
from phorapter.embed import Embedder
from phorapter.errors import TokenizerError
from phorapter.model import Slice, SliceRef
from phorapter.tokens import get_counter

if TYPE_CHECKING:
    from collections.abc import Iterable

    from phorapter.config import Settings
    from phorapter.service.corpora import CorpusService
    from phorapter.service.documents import DocumentService
    from phorapter.service.query import QueryService
    from phorapter.stores import VectorStoreAdapter

__all__ = ["PrefetchSource", "ServiceCore", "StartupReport"]


class PrefetchSource:
    """A synchronous :class:`~phorapter.selection.SliceSource` over a fixed ref map.
...
class ServiceCore:
    ...
    :func:`phorapter.config.build_store` / :func:`phorapter.config.build_embedder`),
    ...
    async def startup(self) -> StartupReport:
        ...
        the outcome, and ``phorapter check`` turns it into an exit code. A store
    ...
    def materialize_counter(self, counter_id: str) -> None:
        ...
        into a :class:`~phorapter.stores.CorpusConfig`: an id that resolves but
